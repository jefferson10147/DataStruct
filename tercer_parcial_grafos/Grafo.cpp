 ...
 *	de Caminos, asigna adem�s sus atributos de tipo de 
 ...
 *	sus valores y instanciando su ListaD de Vertices y 
 *	de Caminos, asigna adem�s sus atributos de tipo de 
 *	de Grafo DIRIGIDO y PONDERADO, adem�s se crea el 
...
/**
 *	@brief retorna determinaci�n acerca de si el
 *	Grafo es dirigido
 *	@since 1.0
...
{
	// si el Grafo no es dirigido
	if( !this->DIRIGIDO ) // Vertices ser� igual al tama�o de la LAP
		return this->lista_adyacencia->getSize();
	else
	{
		// el numero de Vertices inicial sera el tama�o de la LAP
		int vertices = this->lista_adyacencia->getSize();
		Nodo<Vertice>* recor = this->lista_adyacencia->getCab(); // recor ser� cab de LAP
		Nodo<Vertice>* travel = NULL; // travel ser� NULL 
		...
		while( recor )
		{
			...
			while( travel )
			{
				...
				travel = travel->getLink_suc(); // travel ser� su sucesor
			} // fin del while
...
bool Grafo::agregar( Vertice* vertice_partida, Vertice* vertice_destino )
{
	...
	int estado = 1; // estado de la operaci�n de agregar
...
void Grafo::BuscarCaminos()
{
	...
	while( recor )
	{
		...
		// si recor aun tiene direcci�n valida
		if( recor )
		{
...
{
	Nodo<Vertice>* nodo_sublista_adyacencia; // posici�n en la sublista de adyacencia de nodo_agregar
	...
	do
	{
		...
		if( finCamino )
		{
			...
			{
				// nodo_aux_vertice ser� el Vertice en la cab de  
				...
				// agrego a la lista de caminos un nuevo Camino
				this->lista_caminos->agregar( new Camino(), ListaD<Camino>::AGRE_FIN );
				nodo_aux_camino = this->lista_caminos->getFin(); // nodo_aux_camino ser� el ultimo Camino 
				...
				while( true )
				{
					...
					// nodo_aux_vertice ahora ser� su sucesor 	
...
void Grafo::buscarCaminos_maxVertices()
{
	...
	while( recor_vertice )
	{
		...
		if( !this->DIRIGIDO )
		{
			...
			// mientras recor_camino tenga direcci�n valida 
			while( recor_camino )
			{
				...
				if( recor_camino->getInfo()->getVertices() < numeroVertices )
				{
					...
					recor_camino = recor_camino->getLink_suc(); // recor_camino ser� su sucesor 
		...
		else // si no es dirigido
		{
			...
			while( recor_camino )
			{
				...
				// mientras travel_camino tenga direcci�n valida
				while( travel_camino )
				{
					...
					if( *travel_camino->getInfo() < *recor_camino->getInfo() )
					{
						...
						travel_camino = travel_camino->getLink_suc(); // travel_camino ser� sus sucesor
						this->lista_caminos->eliminar( // elimino a eliminar_camino de la lista de 
							ListaD<Camino>::ELIM_ELEMENTO, eliminar_camino->getInfo() ); // Caminos 
					}
 ...
 *	o se omita el par�metro se buscaran Caminos de longitud minima,
...
{
	this->buscarCaminos_maxVertices(); // busca los Caminos con m�s Vertices 
 ...
 *	existiera pero el Grafo fuer� dirigido y este fuera un pozo
 */
bool Grafo::conectividad( Vertice* A, Vertice* B )
{
	...
	{	
		...
		while( recor )
		{
			...
			if( !recor->getInfo()->eliminarListaVerticesDesde( B ) ) 
			{
				...
				recor = recor->getLink_suc(); // recor ser� su sucesor 
	...
	else
	{
		...
		// mientras recor tenga direcci�n valida 
		while( recor )
			{
				...
				// mientras travel tenga direcci�n valida
				while( travel )
				{
						 ...
						 recor->getInfo()->iguales( travel->getInfo() ) )
					{
						...
						travel = travel->getLink_suc(); // travel ser� sus sucesor
 ...
 *	o se omita el par�metro se buscaran Caminos con la menor cantidad de vertices,
...
{
	// recor ser� la cab de la lista de adyacencia principal
	Nodo<Vertice>* recor = this->lista_adyacencia->getCab(); 
